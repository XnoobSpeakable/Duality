<script lang="ts">
    ...
    const logicloop = () => {
        player.time =
            1000 -
            upgrader.getUpgradeTimesBought("upgradetime").toNumber() * 100;
    };
    ...
    (function leftCurrLoop_() {
        leftCurrLoop();
        setTimeout(leftCurrLoop_, player.time);
    })();

    setInterval(
        () => (player.settings.autosaveEnabled ? saveload.save() : null),
        5000
    );
    ...
    function click() {
        ...
        const clickMult = Decimal.dTwo
            .plus(upgrader.getUpgradeTimesBought("upgradeclickmult"))
            .pow(clicksPerClick);
    ...
    <section>
        <div id="left">
            ...
            <div class="upgrade">
                ...
                <p>
                    Cost: {format.big(player.upgrades.upgrademult.cost)}
                    {currencyNames[upgrades.upgrademult.currency]}
                </p>
            </div>
            <br />
            <div class="upgrade">
                ...
                <p>
                    Cost: {format.big(player.upgrades.upgradetime.cost)}
                    {currencyNames[upgrades.upgradetime.currency]}
                </p>
            </div>
        </div>
        ...
        <div id="right">
            ...
            <div class="upgrade">
                <button
                    on:click={() => {
                        upgrader.buyUpgrade("upgradeclickmult");
                    }}
                >
                    increase click multiplier
                </button>
                <p>
                    Cost: {format.big(player.upgrades.upgradeclickmult.cost)}
                    {currencyNames[upgrades.upgradeclickmult.currency]}
                </p>
            </div>
            <br />
        </div>
